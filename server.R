server <- function(input, output) {
    ...
    the_data_fn <- reactive({
        the_data <- read.csv('fav-artists.csv')
        row.names(the_data) <- the_data[,'artist_name']
        return(the_data)
    })  
    ...
    output$tableplot <- renderPlot({
        ...
        tabplot::tableplot(the_data[,2:ncol(the_data)], nBins = nrow(the_data))
    ...
    output$contents <-  DT::renderDataTable({
        the_data <- the_data_fn()
        the_data[,2:ncol(the_data)]
    })
    
    # display a summary of the CSV contents
    output$summary <-  renderTable({
        the_data <- the_data_fn()
        psych::describe(the_data[,2:ncol(the_data)])
    })
    ...
    output$choose_columns_biplot <- renderUI({
        ...
        #colnames <- names(the_data)[2:ncol(the_data)]
        ...
        checkboxGroupInput("columns_biplot", "Choose up to five columns to display on the scatterplot matrix", 
                           ...
                           selected = names(the_data))
    })
    
    # corr plot
    output$corr_plot <- renderPlot({
        the_data <- the_data_fn()
        #the_data <- the_data[2:ncol(the_data)]
        the_data <- the_data[,sapply(the_data,is.numeric)]
